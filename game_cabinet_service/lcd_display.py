...
# gets load of all cpu cores sorted from largest to lowest
def get_cpu_load()->int:
    load_data = cpu.getlastload()
    if load_data:
        load=[]
        for cpu_id in range(0,4):
             load.append(int(load_data['cpu'+str(cpu_id)]))
...
## cpu
  [0x0A, 0x1F,  0x0E,  0x1F,  0x1F,  0x0E,  0x1F,  0x0A]
        ...
        if cpu_load:
            for cpu in cpu_load:
                cpu_load_str += '%0*d%% ' % (2,cpu)
            lcd.lcd_display_string(chr(3)+cpu_load_str,2)
